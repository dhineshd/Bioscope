...
public class ChameleonSession {
    ...
    private ChameleonVideoStream mVideoStream = null;
    ...
    /** You probably don't need to use that directly, use the {@link SessionBuilder}. */
    void addVideoTrack(ChameleonVideoStream track) {
        removeVideoTrack();
        mVideoStream = track;
    }
    ...
    public ChameleonVideoStream getVideoTrack() {
        return mVideoStream;
    }
    ...
    public void postSessionConfigured() {
        mMainHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mCallback != null) {
